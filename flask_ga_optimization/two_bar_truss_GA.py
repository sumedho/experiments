...
def run_ga(spec):
    ...
    pop = generate(2,50, spec)

    for i in range(0,200):
        new_pop = tournament_select(pop, spec)
        pop = best_population(pop,new_pop, spec)
    
    total_time = time.time() - start_time
    total_weight = fitness(pop[0], spec)
    height, diameter = decode(pop[0])
    return total_time, total_weight, height, diameter